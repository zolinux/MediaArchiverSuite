...
class DataServer():
    ...
    def ReserveNextFile(self, maxSize, worker):
        ...
        try:
            self.__lock.acquire()
            c = self.__conn.cursor()
            c.execute("select sourcefiles.id, path, size from sourcefiles join queue on queue.id=sourcefiles.id where sourcefiles.size <= ? and queue.status=0 union select sourcefiles.id, path, size from sourcefiles where sourcefiles.size <= ? and sourcefiles.id not in (select id from queue) union select sourcefiles.id, path, size from sourcefiles join queue on queue.id=sourcefiles.id where sourcefiles.size <= ? and queue.status<0 and queue.status >=-99 and queue.count < 3 LIMIT 1", (maxSize, maxSize, maxSize))
            row = c.fetchone()
